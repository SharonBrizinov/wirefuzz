 ...
 * wirefuzz version — Wireshark v4.4.x and earlier
 *
...
/*
 * v4.4 compatibility: some symbols were added after v4.4.
 */

/* LONGOPT_WSLOG may not exist in very old versions */
#ifndef LONGOPT_WSLOG
...
"binary). These fuzzshark_* targets are also used by oss-fuzz.\n",
	...
	g_setenv("XDG_CACHE_HOME", "/not/existing/directory", 0);
	g_setenv("XDG_CONFIG_HOME", "/not/existing/directory", 0);
	g_setenv("XDG_DATA_HOME", "/not/existing/directory", 0);
...
LLVMFuzzerTestOneInput(const uint8_t *buf, size_t real_len)
{
	...
	rec.rec_header.packet_header.pkt_encap = wirefuzz_encap_type;
	...
	epan_dissect_run(edt, WTAP_FILE_TYPE_SUBTYPE_UNKNOWN, &rec, tvb, &fdlocal, NULL /* &fuzz_cinfo */);

	ws_buffer_free(&pkt_buf);
